...
Surface sdScene(vec3 p){
    vec3 floorColor=vec3(1.+.7*mod(floor(p.x)+floor(p.z),2.));
    Surface co=sdFloor(p,floorColor);
    co=minWithColor(co,sdBox(p,vec3(1),vec3(-4,.5,-4),vec3(1,0,0),identity()));// left cube
    co=minWithColor(co,sdBox(p,vec3(1),vec3(0,.5,-4),vec3(0,.65,.2),identity()));// center cube
    co=minWithColor(co,sdBox(p,vec3(1),vec3(4,.5,-4),vec3(0,.55,2),identity()));// right cube
    return co;
}
...
vec3 calcNormal(in vec3 p){
    ...
    void main()
    {
        ...
        vec2 mouseUV=u_mouse.xy/u_resolution.xy;// Range: <0, 1>
        ...
        if(co.sd>MAX_DIST){
            ...
            float dif=clamp(dot(normal,lightDirection),.3,1.);// diffuse reflection
